...
Mutate provides additional information of how the variable were calculated without interfering with the original columns.
...
Mutate created 'capital_per_year' which represents amount of waste produced per person in a year in kgs, and helped order income levels as used in the X axis.

5.  Describe in your own words what the 'color' argument inside the `aes()` does

It assigns different colors to specified variables such as, income levels in 'income_id' on the plot.

6.  Describe in your own words what the `labs()` function does

It assigns specific labels to X and Y axis.
